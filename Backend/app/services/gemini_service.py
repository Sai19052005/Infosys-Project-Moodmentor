...
class GeminiService:
    def __init__(self):
        ...
        if not GEMINI_API_KEY:
            logging.getLogger(__name__).info(
                "gemini_unconfigured; curated guidance enabled"
            )
            return
        try:
            self.client = genai.Client(
                api_key=GEMINI_API_KEY, http_options={"timeout": 12000}
            )
            logging.getLogger(__name__).info("gemini_configured")
        except Exception as e:
            logging.getLogger(__name__).warning("gemini_init_failed")
    ...
    def generate_empathetic_reply(
        ...
        if self.client is None or not ai_allowed:
            return fallback
        try:
            ...
            logging.getLogger(__name__).warning("gemini_reply_fallback")
            return fallback
