...
class EmotionResult(BaseModel):
    ...
    confidence: float
    valence_score: float
    analysis_method: str = "stored_signal"
...
class RatingInput(BaseModel):
    rating: int = Field(
        ..., ge=-1, le=1, description="1 = helpful 👍, -1 = not helpful 👎, 0 = reset"
    )
...
class RAGSource(BaseModel):
    """A knowledge-base source cited in an AI response."""
...
class QuickMoodInput(BaseModel):
    mood: str = Field(
        ..., pattern="^(joy|sadness|fear|anger|neutral|surprise|disgust)$"
    )
